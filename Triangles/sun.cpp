...
void Sun::initData()
{
	...
	//number of points
	dataCount = (radialStep+1)*heightStep+2+radialStep+1+1; 
	//number of triangles
	unsigned int nTriangles = 2*radialStep*(heightStep-1)+2*radialStep+radialStep+1+1;
	//number of indices
...
auto copy_of_cyl_rad = cylRadius;
	...
	for (unsigned int j=0; j<heightStep; j++)
	{
		...
		for (unsigned int i=0; i<radialStep+1; i++)
		{
			...
			float xPos = cos(fi+j*5)*cos(up);
			float yPos = sin(fi+j*5)*cos(up);
			
			pData[pointId].pos = glm::vec3(xPos, zPos,yPos);
			if (i==0)
				pData[pointId].nor = glm::vec3(xPos, zPos,yPos);
			if (i==radialStep)
				pData[pointId].nor = glm::vec3(xPos, zPos,yPos);
			pData[pointId].nor = glm::vec3(xPos, zPos,yPos);
	...
	//generate round
	{
		unsigned int pointId = heightStep*(radialStep+1)+2;
		pData[pointId].pos = glm::vec3(0,0,0);
		pData[pointId].nor = glm::vec3(0,1,0);
		pData[pointId].tex = glm::vec2(0.5f, 0.5f);		
		...
		for (unsigned int i=0; i<radialStep+1; i++)
		{
			unsigned int pointId = heightStep*(radialStep+1)+3+i;

			float fi = (2*M_PI*(i))/radialStep; //from 0 to 360 degrees
			float xPos = cos(fi);
			float yPos = sin(fi);
			
			pData[pointId].pos = glm::vec3(xPos*2, 0,yPos*2);
	...
	//fill in south pole triangles (last 3*radialStep)
	{
		...
		for (unsigned int i=0; i<radialStep; i++)
		{
			...
			pIndices[startIndex+3*i+0] = heightStep*(radialStep+1)+3+i;
			pIndices[startIndex+3*i+1] = southPoleId;
			pIndices[startIndex+3*i+2] = heightStep*(radialStep+1)+3+i+1;
		}

	}		
	//fill in round
	{
		unsigned int startIndex = 6*radialStep*(heightStep-1)+3*radialStep+3*radialStep;	
		unsigned int southPoleId = heightStep*(radialStep+1)+2;

		for (unsigned int i=0; i<radialStep+1; i++)
		{
			unsigned int pointId = i+heightStep*(radialStep+1)+3;
			pIndices[startIndex+3*i+0] = pointId;
			pIndices[startIndex+3*i+1] = southPoleId;
			pIndices[startIndex+3*i+2] = pointId+1;
			//if (i==radialStep) pIndices[startIndex+3*i+2] = heightStep*(radialStep+1)+3;
		}
